using RoadFlow.Utility.Datas.Sql.Matedatas;
using System.Collections.Generic;
using System.Linq;


namespace RoadFlow.Utility.Datas.Sql.Builders.Core
{
    /// <summary>
    /// 名称项，处理名称中包含符号.
    /// </summary>
    public class NameItem
    {
        ...
        {
            var pattern = "^(([\\[`\"]\\S+?[\\]`\"]).)?(([\\[`\"]\\S+[\\]`\"]).)?([\\[`\"]\\S+[\\]`\"])$";
            var list = RegexHelper.GetValues(name, pattern, new[] { "$1", "$2", "$3", "$4", "$5" }).Select(t => t.Value).ToList();
