...
using RoadFlow.Utility.Datas.Queries.Criterias;
...
namespace RoadFlow.Utility
{
    ...
    public static class QueryableExtensions
    {
        ...
        public static IQueryable<TEntity> WhereIfNotEmpty<TEntity>(this IQueryable<TEntity> source, Expression<Func<TEntity, bool>> predicate) where TEntity : class
        {
            if (source == null)
                throw new ArgumentNullException(nameof(source));
            predicate = RoadFlow.Utility.Datas.Queries.Internal.Helper.GetWhereIfNotEmptyExpression(predicate);
            if (predicate == null)
                return source;
            return source.Where(predicate);
        }
        ...
        public static IQueryable<TEntity> Page<TEntity>(this IQueryable<TEntity> source, IPager pager)
        {
            ...
            RoadFlow.Utility.Datas.Queries.Internal.Helper.InitOrder(source, pager);
            if (pager.TotalCount <= 0)
                pager.TotalCount = source.Count();
            var orderedQueryable = RoadFlow.Utility.Datas.Queries.Internal.Helper.GetOrderedQueryable(source, pager);
