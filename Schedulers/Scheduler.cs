...
using IJob = RoadFlow.Utility.Schedulers.IJob;
using Qz = Quartz;
...
namespace SchedulersUtility
{
    ...
    /// Quartz调度器
    /// </summary>
    public class Scheduler : RoadFlow.Utility.Schedulers.IScheduler
    {
        /// <summary>
        /// Quartz调度器
        /// </summary>
        ...
        public async Task ScanJobsAsync(params Assembly[] assemblies)
        {
            ...
            var jobs = ReflectionHelper.GetInstancesByInterface<IJob>(assemblies);
            if (jobs == null)
                return;
        ...
        private Assembly[] GetAssemblies(Assembly[] assemblies)
        {
            if (assemblies != null && assemblies.Length > 0)
                return assemblies;
            var finder = IocHelper.Create<IFind>();
