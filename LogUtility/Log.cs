...
using LogUtility.Exceptionless;
...
namespace LogUtility {
    ...
    public class Log : LogBase<LogContent> {
        ...
        private static ILogProviderFactory GetLogProviderFactory() {
            try {
                return IocHelper.Create<ILogProviderFactory>();
            }
            catch {
                return new LogUtility.NLog.LogProviderFactory();
            }
        }
        ...
        private static ILogFormat GetLogFormat() {
            try {
                return IocHelper.Create<ILogFormat>();
            }
            catch {
                return LogUtility.Formats.ContentFormat.Instance;
            }
        }
        ...
        private static ILogContext GetLogContext() {
            try {
                return IocHelper.Create<ILogContext>();
            }
            catch {
        ...
        private static ISession GetSession() {
            try {
                return IocHelper.Create<ISession>();
            }
            catch {
                return RoadFlow.Utility.Sessions.Session.Instance;
