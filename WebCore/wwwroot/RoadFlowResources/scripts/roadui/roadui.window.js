        ...
        for (var i = 0; i < iframesArray.length; i++) {
            ...
            if ($maindiv1 != null && $maindiv1.size() > 0) {
                ...
                if (document.all) {
                    //由于IE8下关闭iframe会引起字体图标不显示，这里重新加载CSS
                    try {
                        $("#style_fontawesome", iframesArray[i].contentWindow.document).attr("href", "/RoadFlowResources/scripts/font-awesome-4.7.0/css/font-awesome.min.css?v=" + RoadUI.Core.newid(false));
                    } catch (e) { }
                }
            }
        }
