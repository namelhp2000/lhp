...
; RoadUI.Tab = function (options) {
    var defaults = {
        ...
        paddingleft: "0px", //左padding
        bottomborderwidth:"1px",//底部边框宽度
        maxtabs: -1 //最大tab数量
    };
    this.opts = $.extend(defaults, options);
    this.$tabDiv = null;
    if (this.opts.id.isNullOrEmpty() && this.opts.srcElement === null) {
        throw "要初始化为tab的对象或对象ID为空!";
    }
    else {
    ...
    this.init = function () {
        ...
        var $tabTitle = $('<div class="tab_title"' + (this.opts.bottomborderwidth ? ' style="border-bottom-width:' + this.opts.bottomborderwidth + '"' : '') + '></div>');
        ...
        for (var i = 0; i < tabSize; i++) {
            ...
            if (isClose) {
                $divClose.bind("click", function () { instance.closeTab($(this).parent().attr("itemid")); });
            }
            else {
                $divClose.hide();
            }
            ...
            if (tabDivHeight > 0 && !$tab.attr("isiframe")) {
                $tab.css({ "height": (tabDivHeight - 30).toString() + "px", "overflow": "auto" });
            }
        }
        ...
        if (instance.opts.contextmenu) {
            try {
                ...
                contextMenu.addItem({
                    ico: baseurl + "/RoadFlowResources/images/ico/cross.png",
                    title: "关闭",
                    onclick: function () {
                ...
                contextMenu.addItem({
                    ico: "", title: "除此之外全部关闭", onclick: function () {
                        instance.closeOther();
                    }
                });
                contextMenu.addItem({
                    ico: "", title: "全部关闭", onclick: function () {
                        instance.closeAll();
                    }
                });
                contextMenu.addItem({ type: 1 });
                contextMenu.addItem({
                    ico: baseurl + "/RoadFlowResources/images/ico/Refresh.png",
                    title: "刷新",
                    onclick: function () {
                        instance.refresh();
                    }
    ...
    this.setActive = function (tabID) {
        ...
        for (i = 0; i < $lis.size(); i++) {
            isCurrent = $lis.eq(i).attr("itemid") == tabID;
            ...
            if (isCurrent) {
                $lis.eq(i).find(".tab_title_ul_li_close_div").removeClass().addClass("tab_title_ul_li_close_div1");
            } else {
                $lis.eq(i).find(".tab_title_ul_li_close_div1").removeClass().addClass("tab_title_ul_li_close_div");
            }
        }
        for (i = 0; i < $contents.size(); i++) {
            isCurrent = $contents.eq(i).attr("id") == tabID;
            if (isCurrent) {
                $contents.eq(i).show();
            }
            else {
    ...
    this.addTab = function (options) {
        ...
        var $divClose = $('<div class="tab_title_ul_li_close"><div class="tab_title_ul_li_close_div"></div></div>');
        $divClose.bind("click", function () { instance.closeTab(tabID); });
        if (!opts.close) {
            $divClose.hide();
        }
        $li.append($divText, $divClose);
        ...
        var $content = $('<div id="' + tabID + '"' + (opts.paddingleft ? ' style="padding-left:' + opts.paddingleft + '"' : '') + '></div>');
        if (opts.html) {
