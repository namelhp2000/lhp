...
; RoadUI.Tree = function (options) {
    ...
    this.addNodes = function (json, $node, isRefresh, isInit, isChecked) {
        ...
        for (var i = 0; i < json.length; i++) {
            ...
            var $title = $('<li class="tree_li"><span class="tree_title" style="vertical-align:middle;' + (color && color.length > 0 ? 'color:' + color : '') + '">' + selecthtml + title + '</span></li>');
            $ul.append($title);
    ...
    this.toggleNode = function ($ul, isRefresh) {
        ...
        if (isRefresh || $childNodes.size() === 0) {
            ...
            $.ajax({
                type: "get", url: url, dataType: "json", async: this.opts.async, cache: false,
                success: function (json) {
                    //加载菜单时掉线了，要弹出登录窗口
                    try {
    ...
    var toggleLoading = function ($div, isLoading) {
        if (isLoading) {
            $ico = $(".tree_close", $(">ul", $div));
            $ico.removeClass().addClass("tree_loading");
        }
        else {
            $ico = $(".tree_loading", $(">ul", $div));
