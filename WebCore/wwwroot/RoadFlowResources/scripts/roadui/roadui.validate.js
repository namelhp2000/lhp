...
; RoadUI.Validate = function () {
    ...
    this.validateForm = function (form1, promptType) {
        ...
        if (flag && e && e.srcElement != null && e.srcElement.tagName && "INPUT" === e.srcElement.tagName.toUpperCase()) {
        ...
        for (var i = 0; i < validateArray.length; i++) {
            ...
            switch (validateArray[i].toString().trim().toLowerCase()) {
                ...
                case "datetime":
                    ...
                    break;
                case "let":
                    var let1 = $ele.attr("validate_let") || "0";
                    let1 = parseInt(let);
                    if (!let1) let1 = 0;
                    var errmsg = $ele.attr("errmsg") || "不能小于" + let1 + "个字符";
                    validateFlag1 = this.isLet($ele.val(), let);

                    this.validateCompleted($ele, validateFlag1, promptType, errmsg, true);
                    break;
                case "get":
                    var ge1t = $ele.attr("validate_get") || "0";
                    get1 = parseInt(get1);
                    if (!get1) get1 = 0;
                    var errmsg = $ele.attr("errmsg") || "不能大于" + get1 + "个字符";
                ...
                case "editor":
                    var errmsg = $ele.attr("errmsg") || "不能为空";
                    var html = eval("CKEDITOR.instances." + $ele.attr("id") + ".document.getBody().getText()");
    ...
    this.validateCompleted = function ($element, isSuccess, promptType, errMessage, showInfo) {
        ...
        switch (promptType) {
            ...
            case 2:
                ...
                if ($span.size() == 0 && ($element.attr("type") == "radio" || $element.attr("type") == "checkbox")) {
                    $span = $element.parent().nextAll(".validate,.validate1,.validate2");
                }
                if ($span.size() > 0) {
                    $span = $span.first()
                }
    ...
    {
        return /^13\d{9}$/g.test(val) || (/^15[0-35-9]\d{8}$/g.test(val))
            || (/^17[00-9]\d{8}$/g.test(val)) || (/^18[00-9]\d{8}$/g.test(val)) 
            || (/^19[00-9]\d{8}$/g.test(val));
    };
