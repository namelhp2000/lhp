CKEDITOR.plugins.add('rf_lrselect', {
    requires: ['dialog'],
    init: function (editor) {
        var pluginName = 'rf_lrselect';
        //给自定义插件注册一个调用命令
        ...
        editor.on('doubleclick', function (evt) {
            ...
            if (element.is('input')) {
                ...
                if (type == 'text' && 'lrselect' == element.getAttribute('data-type')) {
        ...
        editor.ui.addButton(pluginName, {
            label: "数据字典选择",
            command: pluginName,
            icon: this.path + "lrselect.png",
            toolbar: 'rf_plugins,14'
        });
            ...
            editor.contextMenu.addListener(function (element) {
                if (element && !element.isReadOnly()) {
                    ...
                    if (name == 'input') {
                        ...
                        if (type == 'text' && 'lrselect' == element.getAttribute('data-type')) {
                            currentSelectEditorElement = element;//保存当前焦点对象 currentSelectEditorElement在index1.jsp中定义
                            return {rf_lrselect: CKEDITOR.TRISTATE_OFF, rf_lrselect_delete: CKEDITOR.TRISTATE_OFF};
                        }
                    }
                }
            });
        ...
        CKEDITOR.dialog.add(pluginName, function (editor) {
            return {
                        ...
                        [
                            {
                                id: pluginName + '_attr',
                                type: 'html',
                                html: '<iframe style="width:100%;height:330px;border:0;" frameborder="0" src="../FormDesignerPlugin/Lrselect?' + query + '"></iframe>'
                            }
                        ]
                }, {
                ...
                onShow: function () {
                    ...
                    function initAttr() {
                        ...
                        if (element) {
                            ...
                            $("#dialogtitle", doc).val(element.getAttribute("data-dialogtitle"));
                            $("#ismore", doc).prop("checked", "1" == element.getAttribute("data-ismore"));
                            $("#isroot", doc).prop("checked", "1" == element.getAttribute("data-isroot"));
                            $("#isparent", doc).prop("checked", "1" == element.getAttribute("data-isparent"));
                            $("#ds_dict_value", doc).val(element.getAttribute("data-ds_dict_value") || "");
                            $("#ds_dict_allchild", doc).prop("checked", "1" == element.getAttribute("data-ds_dict_allchild"));
                ...
                onOk: function () {
                    ...
                    input.setAttribute("value", "数据字典(" + bindfiled + ")");
                    ...
                    input.setAttribute("data-type", "lrselect");

                    input.setAttribute("data-defaultvalue", defaultvalue);
                    input.setAttribute("data-width", width);
                    input.setAttribute("data-dialogtitle", $("#dialogtitle",doc).val()||"");
                    input.setAttribute("data-ismore", $("#ismore", doc).prop("checked")?"1":"0");
                    input.setAttribute("data-isroot", $("#isroot", doc).prop("checked")?"1":"0");
                    input.setAttribute("data-isparent", $("#isparent", doc).prop("checked")?"1":"0");
                    input.setAttribute("data-ds_dict_value", $("#ds_dict_value",doc).val()||"");
                    input.setAttribute("data-ds_dict_allchild", $("#ds_dict_allchild", doc).prop("checked") ? "1" : "0");
