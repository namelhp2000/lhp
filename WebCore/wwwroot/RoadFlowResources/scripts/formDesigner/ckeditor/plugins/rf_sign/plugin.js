CKEDITOR.plugins.add('rf_sign', {
    requires: ['dialog'],
    init: function (editor) {
        var pluginName = 'rf_sign';
        //给自定义插件注册一个调用命令
            ...
            editor.contextMenu.addListener(function (element) {
                if (element && !element.isReadOnly()) {
                    ...
                    if (name == 'input') {
                        ...
                        if (type == 'sign') {
                            currentSelectEditorElement = element;//保存当前焦点对象 currentSelectEditorElement在index1.jsp中定义
                            return {rf_sign: CKEDITOR.TRISTATE_OFF, rf_sign_delete: CKEDITOR.TRISTATE_OFF};
                        }
                    }
                }
            });
        ...
        CKEDITOR.dialog.add(pluginName, function (editor) {
            return {
                        ...
                        [
                            {
                                id: pluginName + '_attr',
                                type: 'html',
                                html: '<iframe style="width:100%;height:400px;border:0;" frameborder="0" src="../FormDesignerPlugin/Button?' + query + '"></iframe>'
                            }
                        ]
                }, {
                ...
                onShow: function () {
                    ...
                    function initAttr() {
                        ...
                        if (element) {
                            ...
                            $("#elementid", doc).val(id);
                            $("#style", doc).val(element.getAttribute("data-style") || "");
                            $("#value", doc).val(element.getAttribute("value") || "");
                        }
                    }
                ...
                onOk: function () {
                    ...
                    var id = $("#elementid", doc).val() || '';
                    if(!id || $.trim(id).length == 0) {
                        id = (table + "-" + RoadUI.Core.newid(false)).toUpperCase();
                    }
                    ...
                    input.setAttribute("value", value || "按钮");
                    if(style) {
